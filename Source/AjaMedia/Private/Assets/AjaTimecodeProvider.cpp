	...
	if (SyncChannel)
	{
		if (State == ETimecodeProviderSynchronizationState::Synchronized)
		{
			...
			if (SyncChannel->GetTimecode(NewTimecode))
			{
				...
				if ((int32)NewTimecode.Frames >= FMath::RoundToInt(GetFrameRate().AsDecimal()))
				{
					UE_LOG(LogAjaMedia, Warning, TEXT("TimecodeProvider %s received an invalid Timecode frame number (%d) for the current frame rate (%s)."), *GetName(), NewTimecode.Frames, *GetFrameRate().ToPrettyText().ToString());
				}
...
FFrameRate UAjaTimecodeProvider::GetFrameRate() const
{
	return bUseReferenceIn ? ReferenceConfiguration.LtcFrameRate : VideoConfiguration.MediaConfiguration.MediaMode.FrameRate;
}
